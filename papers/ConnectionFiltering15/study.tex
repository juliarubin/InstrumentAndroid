...
We refrained from executing functionality related to signing-in into the user's Google Plus account or performing in-app purchases, due to the resign-related limitations mentioned above.
...
\conclusion{To answer RQ1, we conclude that covert communication often occur in real-world applications: 
63\% of the triggered connection statements can be deemed covert.}
...
Analytics services collect information about applications' performance, crash and usage data, as well as the exact actions the user performs within the app. 
...
Beyond A\&A, several applications release information to their own proprietary and third-party services, which does not seem to have any effect on the applications' observable behavior. 
For example, twitter collects information about videos and other rich media attachments followed by the users in tweets. 
The GO Keyboard application sends a set of ids to the \emph{launchermsg.3g.cn} server; it also sends some encrypted data, which we could not decode, to \emph{nextbrowser.goforandroid.com}. 
...
As can be seen in the last column of Table~\ref{tbl:applications}, distinguishing between overt and covert connections cannot be made just by considering their package name schema.  
...
\conclusion{To answer RQ2, we conjecture that covert connections can be detected by inspecting updates to GUI elements on both the success and the failure path of a connection statement. 
Lack of such GUI updates is indicative for a connection being covert for the application execution as the user is unaware of both the success and the failure of the communication.}
