...
\hline
%--------------------------------------------------------------------
Total        &   & 330  & 75 & 52 (69.3\%)   & 25 (48.1\%)  \\ % & 1.8 (30.7\%)  & -
\hline
%--------------------------------------------------------------------
\end{tabular}
}% resizebox
...
When a connection failure occurs, e.g., when the desired server is unavailable, or when a device is put in disconnected or airplane mode, each of these methods throws \texttt{java.io.IOException} exception. 
...
{\bf JR: emph that no judgment, ads are included.}
...
We manually exercised the application for 10 minutes, exploring all its functionality visible to us. 
{\bf JR: mention fetches of data for later updates. Mention coverage.}
We refrained from executing functionality related to signing-in into the user's Google Plus account or performing in-app purchases though, due to the instrumentation-related limitations mentioned above.
...
69\% of the triggered connection statements can be deemed covert.}
...
%The breakdown is similar for the connection statements that we deemed covert: slightly more than 25\%
