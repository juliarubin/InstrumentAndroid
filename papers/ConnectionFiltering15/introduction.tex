...
observed that several popular applications, e.g., Walmart and twitter, spawn services that
communicate with third-party servers even when the application itself is
inactive and the user is unaware that the spawned
...
%executions.  The former is 
(1) the non-exceptional execution reachable after the connection statement
% but limited to processing of the
%Android runtime events that trigger the connection statement.
and 
%The latter traverses methods 
(2) executions up all possible call stacks from the
connection statement until the point
%but stops at points at 
where the exceptions triggered by the connection statement 
%and all related rethrown exceptions 
are cleared.
%that the processing associated with a covert
%connection statements does not directly 
%affect the user experience, i.e, no GIU elements are found 
% %on success
%%or failure, and a covert connection call will not cause
...
The direct processing of the connection is searched for method
