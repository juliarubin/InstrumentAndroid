...
Our technique is not designed to handle
...
\emph{stateful} communication patterns, for which a connection attempt leaves the 
connection target in a state different from the one it has before, and further communication is influenced by the server's state. Our experiments show that such cases of communication with remote servers are rare. 
...
Only 2 connections in total are miss-classified as non-essential: one is responsible for controlling the advertisement material presented by the Google Ads component installed on the device, and another -- for presenting images of additional apps that can be downloaded from Google Play. 
...
Inspired by the high precision of our technique, we apply it on the top 500 popular applications from Google Play. This experiment reveals that xx\% of connection sites encoded in these applications 
