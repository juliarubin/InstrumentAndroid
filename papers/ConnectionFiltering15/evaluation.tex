...
Battery Saver, Spider-Man and Minion Rush games, Microsoft Office Mobile and PicsArt Photo Studio. We conjecture that the last case might me related to resigning issues, but we could not verify that. Other cases stem from the limitations of our static analysis techniques: performing intra-application analysis, ignoring stateful communication and restricting the search for GUI updates only to statements that occur before the success and failure paths unify.
We believe that a low number of such cases, together with the high availability and scalability of our analysis, justifies these choices.   
%\end{itemize}
...
As we analyzed free applications that frequently use open-source packages and then aggregated numbers for
