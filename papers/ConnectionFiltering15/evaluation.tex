...
(column 3 in Table~\ref{tbl:eval}).  A conservative, though feasibly
analyzable, definition of processing related to a connection call and
conservative call graph construction, specifically w.r.t. reflection,
are the major reason we do not achieve higher recall.  Finally, the
analysis is highly efficient and runs in a matter of minutes even on
large applications, as shown in the last column of table
Table~\ref{tbl:eval}.
...
Each pair was given one device with the original and one with the modified versions of the applications. We asked them to execute the same application simultaneously on both devices for 5 to 10 minutes, and to record all differences during that execution.
...
Only 5 applications (10.6\%) where missing essential functionality or seriously broken: Battery Saver, Spider-Man and Minion Rush games, Microsoft Office Mobile and PicsArt Photo Studio. We conjecture that the last case might me related to resigning issues, but we could not verify that. 
