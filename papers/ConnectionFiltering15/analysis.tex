...
begin by defining failure handling, overt and covert connection
...
\item \textsc{\bfseries{Definition (Failure Handling)}}. The failure
  ...
  as the execution paths that start when an exception of type $e$
  propagates to connection call $s$ and end on each path when a
  \lstinline!catch! block is exited such that $e$ and all exceptions
  that could be rethrown on the path from $s$ are caught.
\end{description}

Intuitively, the failure handling of $s$ on $e$ are the computation
...
%\item
\noindent
{\bf 1. User-Interface Cue on Failure}: When $s$ triggers an exception,
  the user may be notified of the failure via a user interface cue
  during failure handling.

\noindent
{\bf 2. Program Exit on Failure}: When $s$ triggers an exception, the
  program may stop executing due to a failure handling path that
  propagates that exception back up to the Android runtime.

\noindent
{\bf 3. User-Interface Cue on Success}: When $s$ succeeds, there
...
%% failure, then $s$ is considered overt.  If it is possible for $e$,
...
API calls that directly call back into the application, e.g.,
\lstinline!Thread.start()!, by replacing the API call with a direct
call to the application method(s) that the API call could invoke.
...
\IIf {(stmt, ex) $\in$ visiting \textbf{or} (stmt, ex) $\in$ overt}
\textbf{return} \EndIIf
...
\State overt $\gets$ overt $\cup$ (stmt, ex)
\State \textbf{return}
\EndIf
...
Analysis starts with the \textsc{FindCatches} procedure listed in
Fig.~\ref{alg:findcatches}.  For each statement and exception pair,
...
caller methods to find \lstinline!catch! blocks that trap
the call graph edge (lines
...
%(line~\ref{alg:native-method-line}).  
...
For connection statement $s$, if the failure handling analysis
