...
invoke a connection call as either {\it essential} or {\it
  non-essential}.  We begin with definitions and our problem
statement:
...
\item{\bf User-Interface Cue on Error}: When $s$ triggers an error,
  the user may be notified of the error via a user interface cue
  during failure handling.

\item {\bf Program Exit on Error}: When $s$ triggers an error, the
...
\item {\bf User-Interface Cue on Success}: When $s$ succeeds, there
  ...
  (b) processing continues beyond the error handling of $s$.
...
\item \textsc{\bfseries{Problem}}: For each connection call in an
  Android application, determine whether it is essential or
  non-essential.
\end{description}
...
interface or could exit the application on error.  We organize the
...
Figure~\ref{alg:findcatches}.  For each statement and exception pair,
$s$ and $e$, respectively, the procedure first consults $s$'s
containing method to find an appropriate \lstinline!catch!; if $e$ is
not caught locally, the analysis recursively visits all direct
predecessors of the method to find \lstinline!catch! blocks that trap
the call statement edge (lines
...
For connection statement $s$, if the failure analysis concludes that
...
merges back to the error paths.
...
\item {\bfseries Success Forward Analysis}: Code reachable from the
  statement immediately after $s$ is searched for a connection call,
  following call statements to analyze their target method(s).  This is
  accomplished by traversing all paths in the interprocedural control
  flow graph (CFG) starting at $s$.  We follow both normal and
  exceptional control paths when analyzing the CFG.  The analysis does
  not follow call graph edges into the Android API.

\item {\bfseries Success Backward Analysis}: For all methods $f$ such
  that there is a path from $f$ to $m$ in the call graph and $f$ was
  searched during the Failure Handler Analysis, examine all statements
...
calls affect the user interface {\it always} on error and {\it
  directly} on success.  The former is examined during the failure
handling analysis and the later is analyzed during the success
analysis.  An execution trace that handles a single runtime event
naturally denotes a unified block of processing, so our analysis
reasons about event handlers to bound the success search to represent
the direct processing of a connection call.

Another insight gleaned from examination of our applications in the
...
by the successful control flow merges back with the exceptional
...
The design our analysis prioritizes high precision over high recall
because we do not want to block connection calls that perform
essential communication.  This strategy guided our decision to employ
CHA for call graph construction and incorporate aggressive heuristics
for capturing reflection.  As will be demonstrated in the next
section, we achieve very high precision for the classification task,
while still achieving recall that blocks a significant number of
invocations of non-essential connection calls.
