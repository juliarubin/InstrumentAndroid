...
\For {nativeEx $\in$ \textsc{GetThrowsException}(succMethd)}
\State \textsc{FindCatches}(meth, stmt, nativeEx, visiting, stack, cg)
\EndFor

\Else
\label{alg:normal-method-call-line}
\State newStack $\gets$ stack
...
\label{alg:rethrown-caught-exception}
\State rethrownTypes $\gets$ rethrownTypes $\cup$ 
...
\textsc{FindCatches} \ref{alg:stack-check-line}.  Searches all possible stacks
